...
with open('02_oliveyoung_listing.csv', mode='r', encoding='utf-8') as f_d:
    reader = csv.reader(f)
    ...
    for row in csv_reader:
        ...
        # 产品名称
        product_code = re.findall(f'goodsNo=(.*?)&', url, re.S)[0]
        dp_name = driver.find_element(By.XPATH, '//*[@id="Contents"]/div[2]/div[2]/div/p[2]').text
        dp_img = driver.find_element(By.XPATH, '//*[@id="mainImg"]').get_attribute('src')

        # 开始判断是否存在 price-1 标签, <strike></strike>是唯一一个price-1标签用到的html标签
        if "strike" in html_data:
                ...
                dp_price2 = driver.find_element(By.XPATH, '//*[@class="price-2"]/strong').text
                dp_price1 = driver.find_element(By.XPATH, '//*[@class="price-2"]/strong').text
        ...
        # sale세일
        if "icon_flag sale" in html_data:
                dp_promo1 = "세일"
        else:
                dp_promo1 = ""

        # 오늘드림
        if "icon_flag delivery" in html_data:
                dp_promo2 = "오늘드림"
        else:
                dp_promo2 = ""

        # 증정
        if "icon_flag gift" in html_data:
                dp_promo3 = "증정"
        else:
                dp_promo3 = ""

        # 1+1
        if "icon_flag plus" in html_data:
                dp_promo4 = "1+1"
        else:
                dp_promo4 = ""

        # 쿠폰
        if "icon_flag coupon" in html_data:
                dp_promo5 = "쿠폰"
        else:
                dp_promo5 = ""
        ...
        ri_1_raw = driver.find_element(By.XPATH, '//*[@id="reviewInfo"]/a/span').text
        ri_1 = re.findall(f'\((.*?)\)', ri_1_raw)[0]

        # Q&A  prd_detail_tab>id=qnaInfo
        qi_1_raw = driver.find_element(By.XPATH, '//*[@id="qnaInfo"]/a/span').text
